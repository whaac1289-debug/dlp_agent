...
std::string g_policy_store_path = "agent/config/policy_snapshot.json";
std::string g_policy_version;
std::mutex g_policy_mutex;
std::string g_agent_jwt;
std::string g_agent_shared_secret;
std::string g_agent_protocol_version = "1.0";
std::string g_config_signature_path = "agent/config/agent_config.json.sig";
...
bool load_config(const char *path) {
    ...
    auto agent_jwt = extract_string(s, "agent_jwt");
    if (!agent_jwt.empty()) g_agent_jwt = agent_jwt;
    auto agent_shared_secret = extract_string(s, "agent_shared_secret");
    if (!agent_shared_secret.empty()) g_agent_shared_secret = agent_shared_secret;
    auto agent_protocol_version = extract_string(s, "agent_protocol_version");
    if (!agent_protocol_version.empty()) g_agent_protocol_version = agent_protocol_version;
    ...
    if (g_rules_path.empty()) {
        g_rules_path = "rules/default_policy.json";
    ...
    if (g_telemetry_spool_path.empty()) {
        g_telemetry_spool_path = "agent/telemetry_spool";
